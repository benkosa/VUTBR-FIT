...
/// @{
VertexIndex gpu_computeGLVertexID(//easy
...
void const *gpu_computeVertexAttributeDataPointer(//easy
  ...
  /// vrcholu a kroku čtecí hlavy.
	...
	return (void const*)(head->offset + (size_t)head->buffer + head->stride * (size_t)gl_VertexID);
}

void gpu_runVertexPuller(GPUVertexPullerOutput *const              output,
                         GPUVertexPullerConfiguration const *const puller,
                         VertexShaderInvocation const vertexShaderInvocation) {//easy
  ...
  ///  - gpu_computeVertexAttributeDataPointer()
	...
	for (size_t i = 0; i < MAX_ATTRIBUTES; i++) {
		const GPUVertexPullerHead *head = &puller->heads[i];
		if (head->enabled != 0)
			output->attributes[i] = gpu_computeVertexAttributeDataPointer(head, id);
		else
...
void gpu_runPrimitiveAssembly(
	...
	GPUVertexPullerOutput puller_output;
	GPUVertexShaderInput  shader_input;

	int i = 0;
	for (i = 0; i < nofPrimitiveVertices; ++i){
		gpu_runVertexPuller(&puller_output, puller, i + baseVertexShaderInvocation);
		shader_input.gl_VertexID = gpu_computeGLVertexID(puller->indices, i + baseVertexShaderInvocation);
		shader_input.attributes = &puller_output;
		vertexShader(&primitive->vertices[i], &shader_input, gpu);
	}
...
void gpu_computeScreenSpaceBarycentrics(
  ...
  /// Normála směřuje směrem dovnitř trojúhelníka.
	...
	Vec2 v0;
	sub_Vec2(&v0, &vertices[1], &vertices[0]);
	Vec2 v1;
	sub_Vec2(&v1, &vertices[2], &vertices[0]);
	Vec2 v2;
	sub_Vec2(&v2, pixelCenter, &vertices[0]);


	float tmp = ((dot_Vec2(&v0, &v0) * dot_Vec2(&v1, &v1)) - (dot_Vec2(&v0, &v1) * dot_Vec2(&v0, &v1)));
	coords->data[1] = ((dot_Vec2(&v1, &v1)*dot_Vec2(&v2, &v0) - dot_Vec2(&v0, &v1) * dot_Vec2(&v2, &v1)) / tmp);
	coords->data[2] = ((dot_Vec2(&v0, &v0)*dot_Vec2(&v2, &v1) - dot_Vec2(&v0, &v1) * dot_Vec2(&v2, &v0)) / tmp);
