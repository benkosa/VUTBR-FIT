...
void phong_onInit(int32_t width, int32_t height) {
	...
	cpu_createBuffers(phong.gpu, 1, &phong.vbo);//vztvorenie bufferov
	cpu_bufferData(phong.gpu, phong.vbo, sizeof(bunnyVertices), bunnyVertices);//vlozenie vrcholov zajaca
	cpu_createBuffers(phong.gpu, 1, &phong.ebo);
	cpu_bufferData(phong.gpu, phong.ebo, sizeof(bunnyIndices), bunnyIndices);

	//nastavenie interpoacie
	...
	int tmp0 = sizeof(float) * 6; //((0 + 1) + (2 + 3))
	int tmp1 = sizeof(float) * 3; //((6 - 3) + (2 + 1) - (0 + 3))
	cpu_createVertexPullers(phong.gpu, 1, &phong.puller);//vztvorenie pulleroa

														 //modifikacia pullerov
	cpu_setVertexPullerHead(phong.gpu, phong.puller, 0, phong.vbo, 0, tmp0);
	cpu_setVertexPullerHead(phong.gpu, phong.puller, 1, phong.vbo, tmp1, tmp0);
	...
	cpu_createBuffers(phong.gpu, 1, &phong.vbo);//vytvorit buffer
	cpu_bufferData(phong.gpu, phong.vbo, sizeof(bunnyVertices), bunnyVertices);//vlozi vrcholy zajca
	cpu_createBuffers(phong.gpu, 1, &phong.ebo);
	cpu_bufferData(phong.gpu, phong.ebo, sizeof(bunnyIndices), bunnyIndices);
}
...
void phong_onDraw(SDL_Surface* surface) {
	...
	cpu_useProgram(phong.gpu, phong.program);//aktivacia shader programu


	cpu_bindVertexPuller(phong.gpu, phong.puller); //aktivacia vertex puller


												   //nahravanie do uniform premennzch
	...
	cpu_drawTriangles(phong.gpu, (sizeof(bunnyIndices) / sizeof(VertexIndex))); //vykreslenie trojuholnikov
