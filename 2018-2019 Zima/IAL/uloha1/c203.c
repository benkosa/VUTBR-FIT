...
*/
	if(q == NULL){//nespravny parameter
		queueError(QERR_INIT);
		return;
	}

	for(int i = 0; i < QUEUE_SIZE; i++)
		q->arr[i] = '*'; //vsetkz hodnotz v poli nasvavi na *
		
	q->f_index = 0;//index na za?tek fronty nastavte na 0,
	q->b_index = 0;//index prvn?o voln?o m?ta nastavte tak?na 0.
...
*/
	  ...
	  //ak prvy a posledny ma rovnaky index
}
...
*/
	  ...
	  //ak je nasledujuci index final index tak je fronta plna
}
...
*/
	...
	if(queueEmpty(q)){ //ak je fronta prazdna
		queueError(QERR_FRONT);
		return;
	}
	
	*c = q->arr[q->f_index]; //vlozime znak na zaciatok fronty
...
*/
	...
	q->f_index = nextIndex(q->f_index); //posunie
...
*/
	if(queueEmpty(q)){ //ak je prazdna fronta
		queueError(QERR_GET);
		return;
	}
	queueFront(q, c); //vratime znak
	queueRemove(q); //odstranime znak
...
*/
	if(queueFull(q)){ //ak je prazdna fronta
		queueError(QERR_UP);
		return;
	}
	
	q->arr[q->b_index] = c; //vlozi znak
	q->b_index = nextIndex(q->b_index); //posunie sa
}
/* Konec p??ladu c203.c */