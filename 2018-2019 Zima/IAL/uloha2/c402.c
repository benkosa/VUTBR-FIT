...
**/
	...
	if (*RootPtr == NULL) {//ked je prazny strom
		//alokacia
		if ((*RootPtr = malloc(sizeof(struct tBTNode))) == NULL)
			return;
		
		(*RootPtr)->Cont = Content;//vlozenie obsau
		(*RootPtr)->LPtr = NULL; //nema dalsie listy
		(*RootPtr)->RPtr = NULL;
		return;
	}

	tBTNodePtr ptr = *RootPtr;//ulozim koren
	tBTNodePtr fptr = NULL;	// ukazatel na otce

	// pokial mame miesto na vkladanie
	while (ptr != NULL){
		// ak je vkladany obsah vacsi ideme do lava
		if (ptr->Cont > Content) {
			fptr = ptr;
			ptr = ptr->LPtr;
		}
		// ak je vkladany obsah mensi ideme do prava
		else if (ptr->Cont < Content) {
			fptr = ptr;
			ptr = ptr->RPtr;
		}
		//ak je vkladany obsah rovnaky
		else 
			return;
	}

	tBTNodePtr newPtr;
	// alokacia aj s error handle
	if ((newPtr = malloc(sizeof(struct tBTNode))) == NULL)
		return;
	
	// inicializacia noveho uzla
	newPtr->Cont = Content;
	newPtr->LPtr = NULL;
	newPtr->RPtr = NULL;
	if (fptr->Cont > Content) //pripojenie z lava
		fptr->LPtr = newPtr;
	else 
		fptr->RPtr = newPtr;//pripojenie z prava
}
...
** a ukazatele na ně si uložíme do zásobníku.
**/

	// dokud neprojdeme vsechny pripustne uzly
	while(ptr != NULL) {
		BTWorkOut(ptr);//zpracujeme
		SPushP(Stack, ptr);//ukazatele ulozime na zasobnik
		ptr = ptr->LPtr;
	}
}
...
**/
	...
	tStackP stackP;//zasobnik
	SInitP(&stackP);

	tBTNodePtr tmp;//pomocny ukazatel

	//zpracujem najlavejsie uzly
	Leftmost_Preorder(RootPtr, &stackP);

	while (SEmptyP(&stackP) == FALSE) {//pokial zasobnik nieje prazdny
		tmp = STopPopP(&stackP); //nacitame zpracovany uzol
		Leftmost_Preorder(tmp->RPtr, &stackP);//zpracujeme najlavejsie uzly, praveho podstromu
	}
}	
...
**/
	...
	while (ptr != NULL) { //pokial neprejdeme vsetky uzly
		SPushP(Stack, ptr); //push uzlu na stack
		ptr = ptr -> LPtr; //ideme do lava
	}
...
**/
 	...
 	tStackP Stack; //zasobnik ukazatelov
	SInitP(&Stack);

  	tBTNodePtr ptr; //pomocny ukazatel
 
	Leftmost_Inorder(RootPtr, &Stack); //ulozenie najlavejsich uzlov
	while (SEmptyP(&Stack) == FALSE) { //pokial este nieco je na stacku
		ptr = STopPopP(&Stack); //nacitame uzol
		BTWorkOut(ptr); //zpracujeme
		Leftmost_Inorder(ptr -> RPtr, &Stack); //ideme do prava
	}
}
...
**/
	...
	while (ptr != NULL) { //pokial prejdeme vsetky uzle
		SPushP(StackP, ptr); //push uzel
		SPushB(StackB, TRUE); //navstivenie uzlu
		ptr = ptr -> LPtr; //ideme do lava
	}
}
...
**/
	...
	//inicializacia premennych a zasobnikou
	tStackP StackP;//zasobnik pointrov
	SInitP(&StackP);

	tStackB StackB;//zasobnik priznakov
	SInitB(&StackB);

	// ulozime najlavejsie uzly a priznaky
	Leftmost_Postorder(RootPtr, &StackP, &StackB);
	while(SEmptyP(&StackP) == FALSE) {//pokial zasobnik nieje prazdny
		RootPtr = STopPopP(&StackP);//vratime vrchol zasobnika
		SPushP(&StackP, RootPtr);

		if (STopPopB(&StackB) == TRUE) {//ak sme sli naposledy do lava
			SPushB(&StackB, FALSE);//pojdeme do prava
			Leftmost_Postorder(RootPtr->RPtr, &StackP, &StackB);
		
		} else {//ak sme sli do prava
			STopPopP(&StackP);//vytiahneme uzol
			BTWorkOut(RootPtr);//zpracujeme ho
		}
	}
}
...
**/
	//ak je strom prazdny nemame co robit
	if (*RootPtr == NULL)
		return;
	...
	SInitP(&stackP);
	Leftmost_Inorder(*RootPtr, &stackP);//prvy priechod do lava
	do {
		tmp = STopPopP(&stackP);//vytiahnem prvy uzol
		*RootPtr = tmp;

		if (tmp->RPtr != NULL)
			Leftmost_Inorder(tmp->RPtr, &stackP);//ulozime si ukazatele do zasobniku
